 ...
 */
enum class NotificationMode(val displayName: String, val emoji: String) {
    HORROR("공포 모드", "👻") {
        override fun getMessages(): List<String> = listOf(
    ...
    /**
     * 각 모드별 메시지 목록을 반환하는 추상 메서드
     */
    abstract fun getMessages(): List<String>

    /**
     * 랜덤 메시지 하나를 반환
     */
    fun getRandomMessage(course: Course): String {
        val message = getMessages().random()
        return message
    ...
    companion object {
        fun fromString(value: String): NotificationMode {
            return values().find { it.name == value } ?: FRESH
        }
    }
}
